...
import { Users, BookOpen, Clock, Shield, ArrowLeft } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { cn } from '@/lib/utils';
import type { PublicCourseDetail } from '@/services';
...
export function CourseHero({ course, onSubscribe, subscribing, isEnrolled, className }: CourseHeroProps) {
            ...
            <div className="lg:col-span-3 space-y-8">
              ...
              >
                ...
                <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10">
                  ...
                  <span className="text-sm text-[rgba(249,246,240,0.75)]">{course.duration} {t('courseDetails.hoursUnit')}</span>
                </div>
              </motion.div>
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span>{t('courseDetails.category')}: {course.categoryName}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Shield className="w-4 h-4 text-[#D4B59E]" />
                  <span>{t('courseDetails.level')}: {t(`teacherPages.courses.levels.${course.level}`)}</span>
                </div>
              </motion.div>
              ...
              <motion.div variants={itemFadeUp} className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#D4B59E]/30 to-[#D4B59E]/30 flex items-center justify-center border border-white/10">
                  <span className="text-lg font-bold text-blue-300">{course.teacherName.charAt(0)}</span>
                </div>
                <div>
                  <p className="text-sm text-[rgba(249,246,240,0.55)]">{t('courseDetails.providedBy')}</p>
                  <p className="text-base font-semibold text-white">{course.teacherName}</p>
                </div>
              </motion.div>
              ...
              <motion.div variants={itemFadeUp} className="flex flex-wrap items-center gap-4 pt-4">
                <motion.button
                  type="button"
                  onClick={onSubscribe}
                  disabled={subscribing || isEnrolled}
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  className="relative group overflow-hidden rounded-2xl bg-gradient-to-r from-[#D4B59E] to-[#C7A187] hover:from-[#D4B59E] hover:to-[#D4B59E] text-white border-0 px-8 py-4 text-base font-bold shadow-lg shadow-blue-600/25 hover:shadow-blue-500/40 transition-all duration-300 cursor-pointer disabled:opacity-60"
                >
                  <span className="relative z-10 flex items-center gap-2">
                    {isEnrolled ? t('courseDetails.alreadyEnrolled') : t('courseDetails.subscribeNow')}
                    {!isEnrolled && <ArrowLeft className="w-4 h-4 group-hover:translate-x-[-4px] transition-transform" />}
                  </span>
              ...
              <div className="relative overflow-hidden rounded-3xl bg-white/[0.04] backdrop-blur-xl border border-white/10 p-8 shadow-lg hover:border-blue-500/30 transition-all duration-500">
                ...
                <div className="relative z-10 space-y-6">
                  ...
                  <motion.button
                    ...
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="relative group cursor-pointer w-full overflow-hidden rounded-2xl bg-gradient-to-r from-[#D4B59E] via-[#C7A187] to-[#C7A187] hover:from-[#D4B59E] hover:via-blue-400 hover:to-[#D4B59E] text-white border-0 px-6 py-4 text-base font-bold shadow-lg shadow-blue-600/25 transition-all duration-300 disabled:opacity-60"
                  >
